...
(function(jQuery){
		...
		var eachfuncs = {
			...
			,printContent: function(elem, item) {
				...
				thisElem.append(
					...
					+'<a href="'+url+'" title="'+artist+' - '+song+'" target="_blank"><img src="'+art+'" class="widget-img-thumb" alt="'+artist+' - '+song+'" onerror="this.src=(\'http://girled.net/js/lastfm/noartwork.gif\')" /></a>'
			...
			,eachThis: function(elem) {
				var thisElem = elem;
				jQuery('#more_track').empty().append('<div style="text-align:right;"><img src="http://girled.net/indi.gif" alt="読み込み中..." width="10" height="10" /> 読み込み中...</div>');
				var thisPage = page;
	...
	jQuery.fn.productSearch = function(trackNum, pageNum) {
		...
		var thisElem = jQuery(this);
		thisElem.children('.ainfo').empty().append('<img src="http://girled.net/indi.gif" alt="商品検索中..." width="10" height="10" /> 商品検索中...');
		var params = {
			'search_index': 'Music',
