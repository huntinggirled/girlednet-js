...
d+'" target="_blank">\u304a\u6c17\u306b\u5165\u308a</a></div></div>');a.children(".one_tweet:last").hover(function(){e(this).children(".reply").fadeTo("normal",1)},function(){e(this).children(".reply").fadeTo("normal",.2)})},relativeTime:function(a){a/=1E3;var b=(new Date).getTime()/1E3;a=parseInt(b-a);a+=60*(new Date).getTimezoneOffset();return 60>a?parseInt(a).toString()+"\u79d2\u524d":3600>a?parseInt(a/60).toString()+"\u5206\u524d":86400>a?parseInt(a/3600).toString()+"\u6642\u9593\u524d":parseInt(a/
...
eachThis:function(a){e("#more_tweet").empty().append('<div style="text-align:right;"><img src="http://'+m+'/js/twitter/indi.gif" alt="\u8aad\u307f\u8fbc\u307f\u4e2d..." width="10" height="10" /> \u8aad\u307f\u8fbc\u307f\u4e2d...</div>');if(1<=d&&g.length>=b.page_count)f.shiftItems(a);else{var c={screen_name:b.screen_name,count:0==d?b.default_count:b.read_count,page:0==d?++d:d++,include_rts:!0};e.ajax({url:"http://"+m+"/js/twitter/twittergw.php",data:c,dataType:"jsonp",callback:"twitterCallback",timeout:5E3}).done(function(c,
